...
/datum/tgui_say/proc/open(payload)
	...
	window_open = TRUE
	saved_text = ""
	if(payload["channel"] != OOC_CHANNEL && payload["channel"] != ADMIN_CHANNEL && payload["channel"] != PRAY_CHANNEL && payload["channel"] != MENTOR_CHANNEL) // Fulp edit - Mentors: Adds MENTOR_CHANNEL
		start_thinking()
